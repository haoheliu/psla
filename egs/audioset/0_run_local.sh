...
do
  ...
  reweight_loss=False
  full_bal=True
  weight_func=calculate_class_weight
  seed=21195
  date=$(date '+%Y-%m-%d_%H_%M_%S')
  sampler=$1

  echo $sampler
  exp_dir=./exp/local-${date}-${dataset}-${sampler}-alpha${alpha}-beta${beta}-hop${hop_ms}-len${target_length}-${reweight_loss}-${preserve_ratio}-${model}-${eff_b}-${lr}-${subset}-impretrain-${impretrain}-fm${freqm}-tm${timem}-mix${mixup}-bal-${bal}-b${batch_size}-git-{$RANDOM}
  mkdir -p $exp_dir
  python ../../src/run.py --data-train $trpath --data-val ./data/datafiles/audioset_eval_data.json \
  --exp-dir $exp_dir --n-print-steps 10 --save_model True --num-workers 16 --label-csv ./data/class_labels_indices.csv \
  --n_class 527 --n-epochs ${epoch} --batch-size ${batch_size} --lr $lr \
  --model ${model} --eff_b $eff_b --impretrain ${impretrain} --att_head ${att_head} \
  --freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} --lr_patience 2 --lambda_zero_loss ${lambda_zero_loss} \
  --dataset_mean -7.4106 --dataset_std 6.3097 --target_length ${target_length} --noise False --hop_ms ${hop_ms} \
  --metrics mAP --warmup True --loss BCE --lrscheduler_start ${lrscheduler_start} --lrscheduler_decay 0.5 \
  --wa True --wa_start ${wa_start} --wa_end ${wa_end} --sampler ${sampler} --preserve_ratio ${preserve_ratio} --val_interval 5 --note ${note} \
