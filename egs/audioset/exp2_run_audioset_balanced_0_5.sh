...
do
  ...
  echo $sampler
  exp_dir=./exp/${date}-{$RANDOM}-${dataset}-${sampler}-alpha${alpha}-beta${beta}-${reweight_loss}-${preserve_ratio}-${model}-${eff_b}-${lr}-${subset}-impretrain-${impretrain}-fm${freqm}-tm${timem}-mix${mixup}-bal-${bal}-b${batch_size}
  mkdir -p $exp_dir
