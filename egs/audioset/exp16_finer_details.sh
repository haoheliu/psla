# Can the LSTM based model predict the resolution better?
...
for preserve_ratio in $3
do
  note=exp16_finer_details
  echo $preserve_ratio$learn_pos_emb
  alpha=1.0
  beta=1.0
  ...
  echo $sampler
  exp_dir=./exp/${dataset}-${sampler}-posemb${learn_pos_emb}-alpha${alpha}-beta${beta}-hop${hop_ms}-${target_length}-${reweight_loss}-${preserve_ratio}-${model}-${eff_b}-${lr}-${subset}-impretrain-${impretrain}-fm${freqm}-tm${timem}-mix${mixup}-bal-${bal}-b${batch_size}-seed${seed}
  mkdir -p $exp_dir
  ...
  --freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} --lr_patience 2 --hop_ms ${hop_ms} \
  --dataset_mean -7.4106 --dataset_std 6.3097 --target_length ${target_length} --noise False --lambda_zero_loss ${lambda_zero_loss} \
  --metrics mAP --warmup True --loss BCE --lrscheduler_start ${lrscheduler_start} --lrscheduler_decay 0.5 --learn_pos_emb ${learn_pos_emb} --apply_zero_loss_threshold ${apply_zero_loss_threshold} \
