...
class DilatedConv1dMaxPoolPlus(AdaSTFT):
    def __init__(self, input_seq_length, preserve_ratio, alpha=1.0, learn_pos_emb=False, mean=-7.4106, std=6.3097):
    ...
    def forward(self, x):
        ...
        mean_feature, max_pool_feature, mean_pos_enc = self.select_feature_fast(x, score, total_length=self.output_seq_length)
        ret['x']=x
...
class DilatedConv1dMaxPoolCh_LinearSpec(AdaSTFT):
    ...
    def forward(self, x):
        ...
        mean_feature = self.normalize(torch.log(mean_feature + 1e-8))
        max_pool_feature = self.normalize(torch.log(max_pool_feature + 1e-8))
...
class DilatedConv1dMaxPoolChIntp(AdaSTFT):
    def __init__(self, input_seq_length, preserve_ratio, alpha=1.0, learn_pos_emb=False, mean=-7.4106, std=6.3097):
        super().__init__(input_seq_length, preserve_ratio, alpha, learn_pos_emb, mean, std)
        self.feature_channels=3
        from models.dilated_convolutions_1d.conv import DilatedConv
...
class DilatedConv1dMaxPoolCh(AdaSTFT):
    def __init__(self, input_seq_length, preserve_ratio, alpha=1.0, learn_pos_emb=False, mean=-7.4106, std=6.3097):
        super().__init__(input_seq_length, preserve_ratio, alpha, learn_pos_emb, mean, std)
        self.feature_channels=3
        from models.dilated_convolutions_1d.conv import DilatedConv
...
if __name__ == "__main__":
    ...
    out1 = test_sampler(DilatedConv1dMaxPoolPlus, data=data)
    out1 = test_sampler(DilatedConv1dMaxPoolCh, data=data)
    out1 = test_sampler(DilatedConv1dMaxPoolCh_LinearSpec, data=data)
    out1 = test_sampler(DilatedConv1dMaxPoolChIntp, data=data)
